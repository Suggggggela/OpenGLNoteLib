...
### 视差贴图
![171x170](assets/5.ParallaxMapping_image_1.png)
...
修改根据的是viewDir和高度图
为了方便理解我们可以看下边的示意图：
![683x235](assets/5.ParallaxMapping_image_2.png)
- 红线 = 假设进行位移后的物体表面
- 黑线 = 实际的物体表面
- 橙色向量 = ViewDir
...
那么如何从A点获取B点的uv呢？
我们可以通过下边的示意图理解：
![566x271](assets/5.ParallaxMapping_image_3.png)
- 我们从高度图获取A点的高度H(A)
...
但问题是当表面变化比较快的时候，B点所在的高度和H(P)相差的就比较大了：![598x238](assets/5.ParallaxMapping_image_4.png)
...
我们可以直接把法线那节的图搬过来看![334x244](assets/5.ParallaxMapping_image_5.png)
因为在这个空间中T和B和uv方向对齐，那么对于P向量x，y分量也就和uv对齐了，这样就不用考虑表面的方向了

### 视差映射
接下来我们看看如何实际进行视差映射
首先我们准备一张高度图![198x198](assets/5.ParallaxMapping_image_6.jpg)
这个高度图和开始时展示的高度图是反相的，这是因为模拟**深度**实际上比模拟**高度**更容易，所以这样的贴图也被称为深度贴图
我们可以反过来看示意图：![525x329](assets/5.ParallaxMapping_image_7.png)
- 用向量V减去A点的uv得到向量P
...
![558x301](assets/5.ParallaxMapping_image_8.png)
- 图中红色线 = 深度图采样到的深度
...
![5.ParallaxMapping_image_9](assets/5.ParallaxMapping_image_9.png)
...
类似这样![444x330](assets/5.ParallaxMapping_image_11.png)
- 在点A和B之间取中间点，得到点1，以此类推直到得到点3

但是这样做在一些表面会出现错误，比如：
![428x313](assets/5.ParallaxMapping_image_12.png)
...
![430x318](assets/5.ParallaxMapping_image_13.png)
...
![432x279](assets/5.ParallaxMapping_image_14.png)
...
![727x545](assets/5.ParallaxMapping_image_15.png)