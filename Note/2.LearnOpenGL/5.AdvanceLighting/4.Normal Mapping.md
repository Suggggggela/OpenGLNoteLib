...
![495x277](assets/4.Normal%20Mapping_image_1.png)
这节我们就了解一下法线贴图
...
![4.Normal Mapping_image_2](assets/4.Normal%20Mapping_image_2.png)

这个颜色正是我们常看到法线贴图的大部分颜色
![213x213](assets/4.Normal%20Mapping_image_3.png)
...
```
效果类似下边这样：
![412x326](assets/4.Normal%20Mapping_image_4.png)
可以看到灯光的效果已经不是平的了

但是如果这时把plane旋转一下，到朝y正方向而不是朝着z轴正方向，效果就会出错了
![406x286](assets/4.Normal%20Mapping_image_5.png)
![413x260](assets/4.Normal%20Mapping_image_6.png)
...
![4.Normal Mapping_image_7](assets/4.Normal%20Mapping_image_7.png)
...
首先TBN三个向量的位置关系如图
![4.Normal Mapping_image_8](assets/4.Normal%20Mapping_image_8.png)
其中T和B两个方向和uv的方向对齐
我们可以找到u和v的两个变化量delta，分别为∆U2 ∆V2，他们与uv也就是TB的方向相同
这两个变化量和另一条边E2组成的三角形的线性组合可以用TB描述：E2 = ∆U2T + ∆V2B
如图：
![4.Normal Mapping_image_9](assets/4.Normal%20Mapping_image_9.png)
E1也是同理：E1 = ∆U1T + ∆V1B
我们也可以通过坐标系的xyz描述：
![516x84](assets/4.Normal%20Mapping_image_10.png)
其中：
- E是两个向量位置的差
-  ∆U 和  ∆V是uv的差
- 需要求解的：T  B
...
通过矩阵乘法的方式表示出来就是下边这样：
![566x69](assets/4.Normal%20Mapping_image_11.png)
左右都乘以∆U ∆V的逆矩阵：
![614x72](assets/4.Normal%20Mapping_image_12.png)
以上就可以求解T B了
逆矩阵可以变化为1/矩阵的行列式，再乘以它的伴随矩阵(Adjugate Matrix)
![4.Normal Mapping_image_13](assets/4.Normal%20Mapping_image_13.png)
...
```
![4.Normal Mapping_image_14](assets/4.Normal%20Mapping_image_14.png)